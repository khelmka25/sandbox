...
namespace sb {
unsigned PrefilterMap::create(unsigned envCubemap, int size, int miplevels) {
  ...
  Shader prefilterShader("prefilter", "assets/shaders/cubemap.vs", "assets/shaders/preprocessor/prefilter.fs");
  prefilterShader.use();
...
unsigned PrefilterMap::loadFromFile(std::filesystem::path filepath) {
  return 0;
}

bool PrefilterMap::saveToFile(unsigned cubemap, int size, int mipLevels, std::filesystem::path path) {
